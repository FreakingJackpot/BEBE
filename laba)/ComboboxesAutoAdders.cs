...
class ComboboxesAutoAdders {
    ...
    public static void FullBrandInf(object sender)
    {
        using (var context = new MYDBCONTEXT())
        {
            var engines = context.Brands.OrderBy(x => x.Id).ToList();
            ComboBox combo = sender as ComboBox;
            combo.Items.Clear();
            foreach (var item in engines) {
    ...
    public static void FullModelInf(object sender)
    {
        using (var context = new MYDBCONTEXT())
        {
            var engines = context.Models.OrderBy(x => x.Id).ToList();
            ComboBox combo = sender as ComboBox;
            combo.Items.Clear();
            foreach (var item in engines) {
    ...
    public static void FullEnginesInf(object sender)
    {
        using (var context = new MYDBCONTEXT())
        {
            var engines = context.Engines.OrderBy(x => x.Id).ToList();
            ComboBox combo = sender as ComboBox;
            combo.Items.Clear();
            foreach (var item in engines) {
                string it = item.Id + " | " + item.Name + " | " + item.FuelType + " | " + item.FuelCapacity + " | " + item.HorsePower;
                combo.Items.Add(it);
            }
        }
    }
    ...
    public static void FullTiresInf(object sender)
    {
        using (var context = new MYDBCONTEXT())
        {
            var engines = context.Tires.OrderBy(x => x.Id).ToList();
            ComboBox combo = sender as ComboBox;
            combo.Items.Clear();
            foreach (var item in engines) {
